...
local function get_modifiers_string(modifiers)
    local result = ''
    local modifier_char = ''
    ...
    for _, modifier in pairs(modifiers) do
        if modifier == "Mod2" then
            goto continue
        end

        modifier_char = string.lower(modifier)
        result = result .. modifier_char .. '-'
        ::continue::
    end
...
local function grabber(_menu, modifiers, key, event)
    if event ~= "press" then return end

    given_modifiers = get_modifiers_string(modifiers)
    key_pressed = given_modifiers .. string.lower(key)

    local sel = _menu.sel or 0
    if gtable.hasitem(menu.menu_keys.up, key_pressed) then
        local sel_new = sel-1 < 1 and #_menu.items or sel-1
        _menu:item_enter(sel_new)
    elseif gtable.hasitem(menu.menu_keys.down, key_pressed) then
        local sel_new = sel+1 > #_menu.items and 1 or sel+1
        _menu:item_enter(sel_new)
    elseif sel > 0 and gtable.hasitem(menu.menu_keys.enter, key_pressed) then
        _menu:exec(sel)
    elseif sel > 0 and gtable.hasitem(menu.menu_keys.exec, key_pressed) then
        _menu:exec(sel, { exec = true })
    elseif gtable.hasitem(menu.menu_keys.back, key_pressed) then
        _menu:hide()
    elseif gtable.hasitem(menu.menu_keys.close, key_pressed) then
...
function menu.entry(parent, args) -- luacheck: no unused args
    ...
    -- Set icon if needed
    local icon, iconbox, icon_init_result
    ...
    if icon then
        ...
        icon_init_result = iconbox:set_image(icon)
        if icon_init_result then
