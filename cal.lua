...
function cal.register(mywidget, custom_current_day_format)
    ...
    if not tooltip then
        ...
        function tooltip:update()
            ...
            state = {month, year}
            tooltip:set_markup(string.format('<span font_desc="Ubuntu mono 20">%s</span>', displayMonth(month, year, 2)))
        end
        tooltip:update()
        --tooltip:set_shape(gears.shape.rounded_rect)
    end
...
function switchMonth(delta)
    ...
    local text = string.format('<span font_desc="Ubuntu mono 20">%s</span>', displayMonth(state[1], state[2], 2))
